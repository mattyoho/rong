...
describe Rong::Elements::GameState do
  it "expects a list of paddle positions and a list ball positions" do
    expect do
      Rong::Elements::GameState.new([0,1,3], [[1,2], [2,3]])
    end.to_not raise_error
  end

  context "paddle positions" do
    let(:game_state) { Rong::Elements::GameState.new([0,1,3,4], [1,2]) }
  ...
  describe "#ball_position" do
    context "when @ball_positions is an array of tuples" do
      let(:game_state) { Rong::Elements::GameState.new([0,1], [[1,2], [3,2]]) }
    ...
    context "when @ball_positions is a tuple" do
      let(:game_state) { Rong::Elements::GameState.new([0,1], [1,2]) }
