...
module RongClient
  ...
  class RongWindow < Gosu::Window
    ...
    def draw
      ...
      ball_box.draw
      if winner_image
        winner_image.draw(WINDOW_CENTER_X - (winner_image.width / 2), WINDOW_CENTER_Y, RongWindow::ZIndex::HUD)
      end
    end
  ...
  class DrawableElement
    ...
    def initialize(window, element, color = Gosu::Color::WHITE, z_index = RongWindow::ZIndex::FOREGROUND)
      @window  = window
      @element = element
      @color   = color
      @z_index = z_index
